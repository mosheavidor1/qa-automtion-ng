...
@app.route('/list_management_content', methods=['GET'])
def list_management_content():
    num_last_content_files = int(request.args.get('num_last_content_files'))
    management_version = request.args.get('management_version')
    all_dirs_in_version = os.listdir(r'X:\Content')
    filtered_nslo = [x for x in all_dirs_in_version if 'FortiEDRContent' in x and management_version in x and 'nslo' in x and 'tmp' not in x]

    filtered_nslo.sort(key=lambda o: int(o.split('FortiEDRContent-')[1].split('-')[0].split('.nslo')[0]))
    filtered_nslo.reverse()
...
@app.route('/dir_content', methods=['GET'])
def get_dir_content():
    location = request.args.get('location')

    if not os.path.exists(fr'x:\{location}'):
        return app.response_class(
            response=json.dumps({
                'message': fr'Unable to access "{location}"'
            }),
            status=404,
            mimetype='application/json',
        )

    content = glob.glob(fr'x:\{location}\\**', recursive=True)
    content = [path.replace(f'x:\\{location}\\', '').replace('\\', '/') for path in content if os.path.isfile(path)]
...
def get_file():
    ...
    if not os.path.exists(fr'x:\{file_path}'):
        return app.response_class(
            response=json.dumps({
                'message': fr'Unable to access "{file_path}"'
            }),
            status=404,
            mimetype='application/json',
        )

    return send_file(fr'x:\{file_path}', as_attachment=True, download_name=os.path.basename(file_path))
