...
from infra.os_stations.linux_station import LinuxStation, TEMP_PATH
from infra.enums import FortiEdrSystemState, LinuxDistroTypes
from infra.system_components.collector import CollectorAgent, FILENAME_EDR_RUNNER, FILENAME_EDR_CONFIG
...
COLLECTOR_INSTALLER_FOLDER_PATH = f"{TEMP_PATH}/version_files"
LINUX_LOCAL_MALWARE_FOLDER_PATH = f"{TEMP_PATH}/malware_simulator"
...
COLLECTOR_BIN_FOLDER_PATH = f"{COLLECTOR_INSTALLATION_FOLDER_PATH}/bin"
...
class LinuxCollector(CollectorAgent):
    def __init__(self, host_ip: str, user_name: str, password: str):
        ...
        self.__qa_files_path = TEMP_PATH
        self._initial_version = self.get_version(safe=True)
        self.datetime_object = datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
    ...
    def get_current_process_id(self):
        ...
        process_id = process_ids[
            0] if process_ids is not None else None  # Why process_ids[0] who told that this is the correct one
    ...
    def get_version(self, safe: bool = False):
        try:
            ...
            version = StringUtils.get_txt_by_regex(text=result,
                                                   regex=r'FortiEDR\s+Collector\s+version\s+(\d+.\d+.\d+.\d+)', group=1)
    ...
    def get_configuration_updated_files_details(self) -> List[dict]:
        raise NotImplemented("should be implemented")

    def get_the_latest_config_update_file_details(self):
        raise NotImplemented("should be implemented")
    ...
    def has_crash(self):
        ...
        if has_crashed:
            ...
            if self.os_station.vm_operations is not None:
                snapshot_name = f"snapshot_with_crash_{time.time()}"
    ...
    def get_agent_status(self):
        ...
        forti_edr_service_status = StringUtils.get_txt_by_regex(text=response, regex=r'FortiEDR\s+Service:\s+(\w+)',
                                                                group=1)
        forti_edr_driver_status = StringUtils.get_txt_by_regex(text=response, regex=r'FortiEDR\s+Driver:\s+(\w+)',
                                                               group=1)
        forti_edr_status = StringUtils.get_txt_by_regex(text=response, regex=r'FortiEDR\s+Status:\s+(\w+)', group=1)
    ...
    def uninstall_collector(self, registration_password=None, stop_collector=True,
        ...
        result = self.os_station.execute_cmd(cmd=cmd, return_output=True, asynchronous=False)
        if expect_to_succeed:
            wait_until_collector_pid_disappears(self)
            self.update_process_id()
        return result

    @allure.step("{0} - Execute FortiEdr installation command")
    def execute_installation_cmd(self, installer_path, is_upgrade: bool = False):
        ...
        if is_upgrade:
            assert "FortiEDR collector updated" in result, f"{self} failed to upgrade collector"
            Reporter.report("Collector upgraded successfully")
        else:
            assert "FortiEDR Collector installed successfully" in result, f"{self} failed to install collector"
            Reporter.report("Collector installed successfully")
    ...
    def install_collector(self,
        ...
        self.execute_installation_cmd(installer_path=installer_path, is_upgrade=False)
        self.configure_collector(aggregator_ip=aggregator_ip,
                                 aggregator_port=aggregator_port,
                                 registration_password=registration_password,
                                 organization=organization)

    @allure.step("{0} - Upgrade FortiEDR Collector")
    def upgrade_collector(self,
                          desired_version: str,
                          registration_password: str):
        """
        To upgrade a linux collector, first stop collector and then install the relevant version without configuring it
        1- Stop linux collector
        2- Install relevant collector version
        3- Update pid after install
        """
        installer_path = self.prepare_version_installer_file(collector_version=desired_version)
        self.stop_collector(password=registration_password)
        self.execute_installation_cmd(installer_path=installer_path, is_upgrade=True)
    ...
    def remove_all_crash_dumps_files(self):
        ...
        if files_paths is not None and isinstance(files_paths, list) and len(files_paths) > 0:
            ...
            for file_path in files_paths:
                if self.os_station.is_file(path=file_path):
                    self.os_station.remove_file(file_path=file_path)
                else:
                    self.os_station.remove_folder(folder_path=file_path)
    ...
    def create_bootstrap_backup(self, reg_password, filename=None):
        bootstrap_filename = COLLECTOR_BOOTSTRAP_FILENAME
        ...
        destination_backup_file_fullpath = f"{TEMP_PATH}/{self.datetime_object}_{COLLECTOR_BOOTSTRAP_FILENAME}.bak"
        source_bootstrap_file_fullpath = f"{COLLECTOR_CONFIG_PATH}/{COLLECTOR_BOOTSTRAP_FILENAME}"

        copy_command = f"cp {source_bootstrap_file_fullpath} {destination_backup_file_fullpath}"
        Reporter.report(f"Running command: {copy_command}", INFO)

        self.os_station.execute_cmd(copy_command)

        return destination_backup_file_fullpath

    def restore_bootstrap_file(self, full_path_filename: str, reg_password: str):
        """This function will restore the bootstrap file from the specified path.

        Args:
        -----
        full_path_filename - filename to restore and place it in the destination directory.

        Returns:
        ------
        destination_bootstrap_file_fullpath - full bootstrap file destination path
        """
        source_bootstrap_file_fullpath = f"{COLLECTOR_CONFIG_PATH}/{COLLECTOR_BOOTSTRAP_FILENAME}"
        destination_bootstrap_file_fullpath = f"{COLLECTOR_CONFIG_PATH}/{COLLECTOR_BOOTSTRAP_FILENAME}"

        if self.get_agent_status is FortiEdrSystemState.RUNNING:
            self.stop_collector(password=reg_password)

        self.os_station.remove_file(source_bootstrap_file_fullpath)

        move_command = f"vm {full_path_filename} {destination_bootstrap_file_fullpath}"
        Reporter.report(f"Running command: {move_command}", INFO)

        self.os_station.execute_cmd(move_command)

        return destination_bootstrap_file_fullpath
    ...
    def config_edr_simulation_tester(self, simulator_path, reg_password):
        ...
        command = fr"python3 {simulator_path}/{FILENAME_EDR_CONFIG} {config_params}"
    ...
    def start_edr_simulation_tester(self, collector, simulator_path):
        scripts_path = r'scripts'
        collector_version = self.get_version()
        json_file_name = f'{collector_version}_{self.datetime_object}.json'
        test_command = f'{collector_version} {self.datetime_object}'
        json_results_file_path = f"{simulator_path}/{json_file_name}"

        # self.os_station.create_new_folder(f"{TEMP_PATH}/Results/CentOS Linux 7") # TODO: remove this when we have proper folder creation from script

        command = fr"python3 {simulator_path}/{scripts_path}/{FILENAME_EDR_RUNNER} {test_command}"
    ...
    def cleanup_edr_simulation_tester(self, edr_event_tester_path, filename):
        ...
        Reporter.report(rf"{COLLECTOR_CONFIG_PATH}/*.*.bak")
        self.os_station.remove_file(rf"{COLLECTOR_CONFIG_PATH}/*.*.bak")

    @allure.step("Try kill FortiEdr process (Hardened)")
    def try_kill_fortiedr_process(self, process_name: str):
        full_pname = f"{COLLECTOR_BIN_FOLDER_PATH}/{process_name}"
        process_ids = self.os_station.get_service_process_ids(full_pname)
        assert len(process_ids) == 1, f"Expecting exactly 1 process id - {full_pname}, actual {len(process_ids)}"

        pid = str(process_ids[0])

        try:
            self.os_station.kill_process_by_id(pid)
            try:
                Reporter.report("Trying to revive to collector by rebooting it and bring services up by OS start up",
                                logger_func=logger.info)
                self.reboot()
            finally:
                assert False, f"Killed process id {pid} successfully, wrong behaviour, should be hardened"
        except Exception as e:
            expected_error_message = "Operation not permitted"
            Reporter.report(f"Failed to kill {full_pname} with the pid - {pid}, got the expected error: {str(e)}",
                            logger_func=logger.info)
            assert expected_error_message.lower() in str(
                e).lower(), f"Did not found {expected_error_message} in error message"
            Reporter.report(f"Protected {process_name} process was not killed as expected", logger_func=logger.info)