...
from infra.system_components.collectors.windows_os.windows_collector_installation_utils import (
    ...
    get_installer_path,
    generate_installation_cmd, generate_upgrade_cmd
)
...
FORTI_EDR_DRIVERS_FOLDERS = "C:\\Windows\\System32\\drivers"
...
class WindowsCollector(CollectorAgent):
    ...
    def __init__(self, host_ip: str, user_name: str, password: str):
        ...
        self.__collector_service_exe: str = fr"{self.__collector_installation_path}\FortiEDRCollectorService.exe"
        ...
        self.__target_logs_folder: str = r"C:\ParsedLogsFolder"
        ...
        self.__collector_logs_folder: str = fr"{self.__program_data}\Logs"
        self.__collector_config_folder: str = fr"{self.__program_data}\Config\Collector"
    ...
    @property
    def collector_bootstrap_filename(self) :
        return self.__bootstrap_file_name
    ...
    def get_the_latest_config_update_file_details(self) -> dict:
        """
        return these data of the latest created configuration file: names, sizes, datetime
        """
        config_files_details = self.get_configuration_updated_files_details()  # sorted by timestamp
    ...
    def wait_for_new_config_file(self, config_files_details_before_action: List[dict] = None):
        ...
        """
        config_files_details_before_action = config_files_details_before_action or self.get_configuration_updated_files_details()
        ...
        def condition():
            updated_latest_config_file_details = self.get_the_latest_config_update_file_details()
    ...
    def get_version(self, safe: bool = False):
        try:
            ...
            version = StringUtils.get_txt_by_regex(text=result, regex=r'FortiEDR\s+Collector\s+Service\s+version\s+(\d+.\d+.\d+.\d+)', group=1)
        ...
        for single_folder in folder_to_search:
            ...
            if is_folder_exist:
                ...
                if files_in_folder is not None and len(files_in_folder) > 0:
                    ...
                    if r'C:\ProgramData\FortiEDR\Dumps\Collector\TMP' in files_in_folder:
                        files_in_folder.remove(r'C:\ProgramData\FortiEDR\Dumps\Collector\TMP')

                    if single_folder == r'C:\ProgramData\FortiEDR\Dumps\Collector':
                        files_in_folder = [file for file in files_in_folder if not (file.endswith('.exe.mem') or file.endswith('.e'))]
    ...
    def get_agent_status(self) -> FortiEdrSystemState:
        ...
        forti_edr_service_status = StringUtils.get_txt_by_regex(text=response, regex=r'FortiEDR\s+Service:\s+(\w+)', group=1)
        forti_edr_driver_status = StringUtils.get_txt_by_regex(text=response, regex=r'FortiEDR\s+Driver:\s+(\w+)', group=1)
    ...
    def upgrade_collector(self,
                          ...
                          registration_password: str = None):
        registration_pass = registration_password or REGISTRATION_PASS
        installer_path = get_installer_path(collector=self, version=desired_version)
        logs_path = self._get_logs_path(collector=self, prefix="Installation_logs")
        Reporter.report(f"Upgrade logs can be found here: {logs_path}")

        cmd = generate_upgrade_cmd(installer_path=installer_path,
                                   registration_pass=registration_pass,
                                   logs_path=logs_path)
        self.os_station.execute_cmd(cmd=cmd, fail_on_err=True)
        wait_until_collector_pid_appears(self)
    ...
    def uninstall_collector(self, registration_password: str = None,
                            stop_collector: bool = False,
    ...
    def get_exe_folders_paths_by_name(self, exe_name: str) -> List[str]:
        """ Different versions of the same app can be installed in different folders, we can find these folders by
         finding the paths of the relevant exe binaries, for example when searching for the winscp.exe it can return
         that this exe found in the downloads folder and in the program files folder (=that means that winscp is
         installed in these folders)"""
        if not exe_name.endswith('.exe'):
            raise Exception(f"Expected name with .exe extension, we got {exe_name}")
        cmd = fr'dir C:\{exe_name} /S /B'
        paths = self.os_station.execute_cmd(cmd=cmd)
        if paths is not None and 'file not found' not in paths.lower() and 'the system cannot find the file specified' \
                not in paths.lower():
            logger.info(f"{exe_name} was found in paths: {paths}")
            return paths.replace(fr'\{exe_name}', '').split("\r\n")
        return []
    ...
    def create_bootstrap_backup(self, reg_password, filename=None):
        ...
        expected_backup_filename = fr"{self.__qa_files_path}\{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_{bootstrap_filename}"
    ...
    def restore_bootstrap_file(self, full_path_filename: str, reg_password: str):
        ...
        backedup_config_file_content = self.os_station.get_file_content(full_path_filename)

        move_to_config_path = fr"move {full_path_filename} {self.collector_config_folder}\{self.__bootstrap_file_name}"
        result = self.os_station.execute_cmd(move_to_config_path)
        logger.debug(fr"Renamed and moved file '{full_path_filename}', {result}")

        try:
            logger.info("Reying to read file content")
            replaced_config_file_content = self.os_station.get_file_content(fr"{self.collector_config_folder}\{self.__bootstrap_file_name}")
        except Exception as e:
            logger.exception(f"Exception occured: {e}, trying to rename bak file")
            self.os_station.execute_cmd(fr"rename {self.collector_config_folder}\{self.__bootstrap_file_name}.bak {self.collector_config_folder}\{self.__bootstrap_file_name}")
            replaced_config_file_content = self.os_station.get_file_content(fr"{self.collector_config_folder}\{self.__bootstrap_file_name}")

        assert backedup_config_file_content == replaced_config_file_content, "Bootstrap configuration files not replaced."

        self.start_collector()


    @allure.step("Simulation of EDR events configuration")
    def config_edr_simulation_tester(self, simulator_path: str, reg_password: str):
        """
        This functions configures the EDR tester for running the EDR simulation in the collector.
        """
        config_params = 'config -a'
        command = fr"python {simulator_path}\{FILENAME_EDR_CONFIG} {config_params}"
    ...
    def start_edr_simulation_tester(self, collector, simulator_path):
        ...
        json_file_name = f'{collector_version}_{datetime_object}.json'
        test_command = f'{collector_version} {datetime_object}'
        scripts_path = r'\scripts'
        json_results_file_path = f"{simulator_path}\\{json_file_name}"

        command = fr"python {simulator_path}{scripts_path}\{FILENAME_EDR_RUNNER} {test_command}"

        Reporter.report(f"Running command: '{command}'", INFO)
        Reporter.report(f"Timeout for the command: {EDR_EVENT_TESTER_TIMEOUT}(seconds)", INFO)
    ...
    def cleanup_edr_simulation_tester(self, edr_event_tester_path, filename):
        ...
        self.os_station.remove_folder(folder_path=rf"{self.__qa_files_path}\Results")
    ...
    @allure.step("Get process name and returns pid")
    def find_pid_by_name(self, name):
        raise NotImplemented("should be implemented")

    @allure.step("Get pid and kill this process")
