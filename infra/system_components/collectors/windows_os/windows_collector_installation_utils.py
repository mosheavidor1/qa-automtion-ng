...
def get_installer_path(collector, version):
    ...
    blg2log_name = f'blg2log_{version}.exe'
    ...
    installation_files_folder = collector.os_station.copy_files_from_shared_folder(
        ...
        files_to_copy=[installer_file_name, blg2log_name])
...
def generate_upgrade_cmd(installer_path, registration_pass, logs_path):
    cmd = rf'msiexec /i "{installer_path}" /qn PWD={registration_pass} /LIME {logs_path}'
    Reporter.report(f"cmd for upgrade: {cmd}")
    return cmd
