...
class FortiEdrLinuxStation(LinuxStation):
    ...
    def get_status(self):
        ...
        result = self.execute_cmd(cmd=cli_cmd, fail_on_err=False, return_output=True, attach_output_to_report=True)
    ...
    def clear_logs(self, file_suffix='.log'):
        ...
        for single_file in files:
            ...
            if self.__component_type == ComponentType.AGGREGATOR or self.__component_type.MANAGEMENT:
                ...
                result = StringUtils.get_txt_by_regex(text=single_file, regex=fr'{file_suffix}.(\d+)', group=1)
    ...
    @allure.step("Search if the string: {string_to_search} exist is log")
    def is_string_exist_in_logs(self,
        ...
        logs_dict = self.get_logs_after_given_timestamp(first_log_timestamp=first_log_timestamp,
        ...
        if logs_dict is None or len(logs_dict) == 0:
            return False
        for file_name, file_content in logs_dict.items():
            result = StringUtils.get_txt_by_regex(text=file_content.lower(),
                                                  regex=string_to_search.lower(),
                                                  group=0)
            if result is not None:
    ...
    @allure.step("Get that created after: {first_log_timestamp}")
    def get_logs_after_given_timestamp(self,
                                       first_log_timestamp: str,
                                       machine_timestamp_date_format: str,
                                       log_timestamp_date_format: str,
                                       log_timestamp_date_format_regex_linux: str,
                                       log_file_datetime_regex_python: str):
        """
        This method will return log files and content after a given time stamp as dict
        return dict with name = file name, value = file content
        """
        logs_dict = {}
        ...
        if all_log_files is None or all_log_files == []:
            Reporter.report(f"There are no logs that was created after {first_log_timestamp}")
        ...
        for single_file in all_log_files:
            ...
            if lines_with_matching_date is not None and len(lines_with_matching_date) > 0:
                ...
                for single_line in tmp_content_splitted:
                    line_date = StringUtils.get_txt_by_regex(text=single_line,
                                                             regex=f'({log_file_datetime_regex_python})',
                                                             group=1)
    ...
    def append_logs_to_report_by_given_timestamp(self,
        ...
        if len(logs_dict) > 0:
            for log_file_name, content in logs_dict.items():
                Reporter.attach_str_as_file(file_name=log_file_name, file_content=content)
            del logs_dict
    ...
    def upgrade_to_specific_build(self,
        ...
        base_version = StringUtils.get_txt_by_regex(text=current_version, regex=r"(\d+.\d+.\d+)", group=1)
        current_build = StringUtils.get_txt_by_regex(text=current_version, regex=r"\d+.\d+.\d+.(\d+)", group=1)
        ...
        if desired_build is None:
            ...
            desired_build = StringUtils.get_txt_by_regex(text=desired_build, regex=r"\d+.\d+.\d+.(\d+)", group=1)

        if int(desired_build) > int(current_build):

            if len(str(desired_build)) < 4 and Version(current_version) > Version('6.0.0.0'):
                desired_build = str(desired_build).rjust(4, '0')
