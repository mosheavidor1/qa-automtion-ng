...
from infra.utils.utils import StringUtils, HttpRequesterUtils, HttpRequestMethodsEnum
from infra.common_utils import wait_for_condition
from .linux_distros import LinuxDistroDetails
import logging
logger = logging.getLogger(__name__)

INTERVAL_STATION_KEEPALIVE = 1
WAIT_FOR_STATION_UP_TIMEOUT = 4 * 60
WAIT_FOR_STATION_DOWN_TIMEOUT = 2 * 60
TEMP_PATH = "/tmp"
COLLECTOR_EDR_EVENT_TESTER_PATH = f"{TEMP_PATH}/edr_event_tester"
...
class LinuxStation(OsStation):
    ...
    def __init__(self, host_ip: str, user_name: str, password: str):
        ...
        self.distro_data.update_version(self.os_version)
    ...
    def _is_lsb_release_installed(self) -> bool:
        """ Checks if lsb_release utility is installed on the machine """
        cmd = 'which lsb_release'
        result = self.execute_cmd(cmd=cmd, return_output=True)
        return result and 'no lsb_release' not in result

    def get_os_version(self):
        if self._is_lsb_release_installed():
            cmd = 'lsb_release -r'
            regex = r'Release:\s+(.*)'
        else:
            cmd = 'hostnamectl'
            regex = r'Operating\s+System:\s+.*?(\d+)'
        result = self.execute_cmd(cmd=cmd, return_output=True)
        ver = StringUtils.get_txt_by_regex(text=result, regex=regex, group=1)
        return ver

    def get_os_name(self):
        if self._is_lsb_release_installed():
            cmd = 'lsb_release -i'
            regex = r'Distributor ID:\s+(.*)'
        else:
            cmd = 'hostnamectl'
            regex = r'Operating\s+System:\s+(.+)'
        result = self.execute_cmd(cmd=cmd, return_output=True)
        os_name = StringUtils.get_txt_by_regex(text=result, regex=regex, group=1)
        return os_name
    ...
    def reboot(self):
        ...
        cmd = 'shutdown --no-wall 0 2> /dev/null'
        self.execute_cmd(cmd=cmd, fail_on_err=True, return_output=False, attach_output_to_report=True)
        self.wait_until_machine_is_unreachable()
        while not self._vm_operations.is_power_off():
            time.sleep(5)
        self._vm_operations.power_on()
        try:
            self.wait_until_machine_is_reachable(0)
        except Exception as e:
    ...
    def get_file_content(self, file_path, filter_regex: str = None):
        ...
        Reporter.report(f"output size: {len(output)}", logger_func=logger.info)
        return output
    ...
    def kill_process_by_id(self, pid):
        cmd = f"kill -9 {pid}"
        result = self.execute_cmd(cmd=cmd, fail_on_err=True, return_output=True)
        return result
    ...
    def mount_shared_drive_locally(self, desired_local_drive: str, shared_drive: str, user_name: str, password: str):
        extra_options = ''
        if self.distro_data.version_name in ['Ubuntu16.04', 'CentOS7']:
            extra_options = 'vers=2.0,'
        cmd = (
            f"sudo mount -t cifs -o {extra_options}username={user_name},password={password} "
            f"{shared_drive} {desired_local_drive}"
        )
    ...
    def copy_folder(self, source: str, target:str):
        raise NotImplementedError("Copy folder is not implemented yet")
    ...
    def _get_files_from_shared_folder_by_download(
            self,
        ...
        shared_drive_path_for_command = shared_drive_path.replace(third_party_details.SHARED_DRIVE_VERSIONS_PATH, '')
        shared_drive_path_for_command = shared_drive_path_for_command.replace('\\', '/')
        if shared_drive_path_for_command[0] == '/':
            shared_drive_path_for_command = shared_drive_path_for_command[1:]

        if '*' in files_to_copy:
            content = HttpRequesterUtils.send_request(
                HttpRequestMethodsEnum.GET,
                f'{third_party_details.AUTOMATION_SERVICES_URL}'
                f'/dir_content?location={shared_drive_path_for_command}',
            )
            files_to_copy.remove('*')
            files_to_copy = list(set(files_to_copy + content['content']))

        for file_name_to_copy in files_to_copy:
            cmd = (
                f'wget {third_party_details.AUTOMATION_SERVICES_URL}'
                f'/get_file?file={shared_drive_path_for_command}/{file_name_to_copy} '
                f'-O {target_folder}/{file_name_to_copy}'
            )
            self.execute_cmd(cmd)

        return target_folder

    def _get_files_from_shared_folder_by_mount(
            self,
            target_path_in_local_machine: str,
            shared_drive_path: str,
    ...
    def copy_files_from_shared_folder(self,
        ...
        :param shared_drive_user_name: username
        ...
        if self.distro_data.version_name == 'CentOS6':
            # centOS6 cannot mount the sharepoint because of the kernel does not support the correct version
            target_folder = self._get_files_from_shared_folder_by_download(
                target_path_in_local_machine,
                shared_drive_path,
                files_to_copy,
            )
        else:
            target_folder = self._get_files_from_shared_folder_by_mount(
                target_path_in_local_machine,
                shared_drive_path,
                shared_drive_user_name,
                shared_drive_password,
                files_to_copy,
            )
        return target_folder
    ...
    def extract_compressed_file(self, file_path_to_extract=None, file_name=None):
        ...
        """
        sub_folder = file_name.split('.')[0]
        output_path = f"{TEMP_PATH}/{sub_folder}"

        if self.is_path_exist(output_path):
            self.remove_folder(output_path)
            logger.info(f"Path removed: {output_path}")

        cmd = f'unzip {file_path_to_extract}/{file_name} -d {output_path}'
        Reporter.report(f"Running command: {cmd}")

        output = self.execute_cmd(f"{cmd}")
        Reporter.attach_str_as_file(file_name=f"unzip-{sub_folder}", file_content=output)
    ...
    def rename_hostname(self, host_name: str):
        Reporter.report(f'Renaming host to: "{host_name}"')
        cmd = f'hostnamectl set-hostname {host_name}'
        if self.distro_data.version_name == 'CentOS6':
            cmd = fr"sed -i 's/\(HOSTNAME=\).*/\1{host_name}/g' /etc/sysconfig/network"
        elif self.distro_data.version_name == 'Ubuntu16.04':
            cmd = fr"{cmd}; sed -i 's/\(127.0.1.1\s\).*/\1{host_name}/g' /etc/hosts"
        self.execute_cmd(cmd=cmd)
        self.reboot()
