...
class LinuxDistroDetails:
    ...
    def update_version(self, os_version: str):
        if self.distro_type == LinuxDistroTypes.UBUNTU:
            self.version_name = f'Ubuntu{os_version}'
        elif self.distro_type == LinuxDistroTypes.CENTOS:
            self.version_name = f'CentOS{os_version.split(".")[0]}'
