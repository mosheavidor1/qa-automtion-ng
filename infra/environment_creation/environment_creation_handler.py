...
class EnvironmentCreationHandler:
    ...
    def _deploy_components(
        ...
        for single_comp in components:
            if isinstance(single_comp, Core):
                single_comp.stop_service()

        for single_comp in components:
            if isinstance(single_comp, Aggregator):
                single_comp.restart_service()

        time.sleep(10)

        for single_comp in components:
            if isinstance(single_comp, Core):
                single_comp.start_service()
    ...
    def deploy_system_component_from_template(
        ...
        match component_type:
            case ComponentType.MANAGEMENT | ComponentType.BOTH:
                ...
                default_organization.update_name(new_name=sut_details.default_organization_name)
...
if __name__ == "__main__":
    ...
    #
    # name, vm_manager = EnvironmentCreationHandler.deploy_system_component_from_template(
    ...
    # name, vm_aggregator = EnvironmentCreationHandler.deploy_system_component_from_template(
    #     vsphere_cluster_handler=_vsphere_handler,
    ...
    name, vm_core = EnvironmentCreationHandler.deploy_system_component_from_template(
        vsphere_cluster_handler=_vsphere_handler,
