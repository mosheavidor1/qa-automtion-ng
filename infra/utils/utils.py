...
class StringUtils:
    ...
    @staticmethod
    def is_str_contains_upper_case_chars(text: str):
        return StringUtils.is_chars_exist_by_regex(text=text, regex='^(?=.*[A-Z]).+$')

    @staticmethod
    def is_str_contains_lower_case_chars(text: str):
        return StringUtils.is_chars_exist_by_regex(text=text, regex="^(?=.*[a-z]).+$")

    @staticmethod
    def is_str_contains_special_chars(text: str):
        return StringUtils.is_chars_exist_by_regex(text=text, regex="^(?=.*[-+_!@#$%^&*., ?]).+$")

    @staticmethod
    def is_str_contains_numeric_chars(text: str):
        return StringUtils.is_chars_exist_by_regex(text=text, regex=r"^(?=.*\d).+$")

    @staticmethod
    def is_chars_exist_by_regex(text: str, regex: str) -> bool:

        if regex is None or regex == '':
            raise Exception("regex should not be empty or None")
