...
class AdministratorRest(BaseRestFunctionality):
    ...
    def get_system_license_blob(self, expected_status_code: int = 200):
        url = '/admin/list-system-summary'
        params = {
            "organization": "All organizations",
            "addLicenseBlob": True
        }
        status, response = self._rest.passthrough.ExecuteRequest(url=url, mode="get", inputParams=params)
        err_msg = f"Failed to get system summary with license blob, got error: {response}"
        assert status, err_msg
        self._validate_expected_status_code(expected_status_code=expected_status_code,
                                            actual_status_code=response.status_code,
                                            error_message=err_msg)
        system_summary_with_license_blob = json.loads(response.text)
        license_blob = system_summary_with_license_blob["licenseBlob"]
        return license_blob
