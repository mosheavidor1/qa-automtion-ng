...
class CommunicationControlRest(BaseRestFunctionality):
    ...
    def assign_policy(self, policy_name, group_name, force_assign=True, expected_status_code: int = 200):
        """ force_assign indicates whether to force the assignment even if the group is assigned to similar policies """
        url = '/comm-control/assign-collector-group'
        params = {'collectorGroups': group_name, 'policyName': policy_name, 'forceAssign': force_assign}
    ...
    def get_applications(self) -> List[dict]:
        status, response = self._rest.commControl.ListProducts(itemsPerPage=999)
        assert status, f"Failed to get comm control applications, got error: {response}"
        applications = json.loads(response.text)
        return applications

    @allure.step("API- Set comm control application {applications} policies {policy_names} to permission {permission}")
    def set_applications_permission(self, policy_names, applications, versions, permission, expected_status_code: int = 200):
        status, response = self._rest.commControl.SetApplicationMode(products=applications,
                                                                     versions=versions,
                                                                     policies=policy_names,
                                                                     decision=permission)
        assert status, f"Failed to set comm control application {applications} policies {policy_names} to permission {permission}"
        self._validate_expected_status_code(expected_status_code=expected_status_code,
                                            actual_status_code=response.status_code,
                                            error_message=f"Failed to set comm control application {applications} policies {policy_names} to permission {permission}")

    @allure.step("API- Resolve/Unresolve application {applications} in version {versions}")
    def resolve_or_unresolve_app(self, applications, to_resolve, versions, expected_status_code: int = 200):
        url = '/comm-control/resolve-applications'
        params = {'products': applications, 'resolve': to_resolve, 'versions': versions}
        status, response = self._rest.passthrough.ExecuteRequest(url=url, mode="put", inputParams=params)
        assert status, f"Failed to resolve/un-resolve application {applications} in version {versions}"
        self._validate_expected_status_code(expected_status_code=expected_status_code,
                                            actual_status_code=response.status_code,
                                            error_message=f"Failed to resolve/un-resolve application {applications} in version {versions}")
