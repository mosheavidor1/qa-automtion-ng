...
class SecurityPoliciesFactory(BaseApiObjFactory):
    """ Find security policies and return them as rest objects  with the user's credentials.
    ...
    def get_by_name(self, policy_name, rest_client=None, safe=False) -> SecurityPolicy:
    ...
    def get_by_field(self, field_name, value, rest_client=None, safe=False) -> List[SecurityPolicy]:
        ...
        for policy_fields in policies_fields:
            if policy_fields[field_name] == value:
                policy = SecurityPolicy(rest_client=rest_client, initial_data=policy_fields)
    ...
    def get_all(self, rest_client=None, safe=False) -> List[SecurityPolicy]:
        policies = []
        rest_client = rest_client or self._factory_rest_client
        org_name = self._organization_name
