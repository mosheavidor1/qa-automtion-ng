...
DEFAULT_TITLE = "title"
TEMP_PASSWORD = "TempPassword_12"
DEFAULT_PASSWORD = f"{TEMP_PASSWORD}_1"


class UserRolesLegacy(Enum):
    USER = 'User'
    ADMIN = 'Admin'
...
LOCAL_ADMIN_ROLES_LEGACY = [UserRolesLegacy.USER.value, UserRolesLegacy.LOCAL_ADMIN.value, UserRolesLegacy.REST_API.value]


class UserFieldsNamesLegacy(Enum):
    ID = 'id'
    USERNAME = 'username'
...
class UserLegacy(BaseApiObj):
    ...
    def __init__(self, password: str, initial_data: dict):
        ...
        self._id = initial_data[UserFieldsNamesLegacy.ID.value]  # Static, unique identifier
        self._organization_name = initial_data[UserFieldsNamesLegacy.ORGANIZATION.value]
        super().__init__(rest_client=RestCommands(management_ip=sut_details.management_host,
                                                  rest_api_user_name=initial_data[UserFieldsNamesLegacy.USERNAME.value],
    ...
    def create(cls, rest_client: RestCommands, username, user_password, organization_name, roles,
        ...
        user_data = {
            UserFieldsNamesLegacy.USERNAME.value: username,
            UserFieldsNamesLegacy.ORGANIZATION.value: organization_name,
            UserFieldsNamesLegacy.PASSWORD.value: TEMP_PASSWORD,
            UserFieldsNamesLegacy.ROLES.value: roles,
            UserFieldsNamesLegacy.FIRST_NAME.value: optional_data.get(UserFieldsNamesLegacy.FIRST_NAME.value, DEFAULT_FIRST_NAME),
            UserFieldsNamesLegacy.LAST_NAME.value: optional_data.get(UserFieldsNamesLegacy.LAST_NAME.value, DEFAULT_LAST_NAME),
            UserFieldsNamesLegacy.EMAIL.value: optional_data.get(UserFieldsNamesLegacy.EMAIL.value, DEFAULT_MAIL),
            UserFieldsNamesLegacy.TITLE.value: optional_data.get(UserFieldsNamesLegacy.TITLE.value, DEFAULT_TITLE)
        }
    ...
    def get_username(self, from_cache=None, update_cache=True):
        field_name = UserFieldsNamesLegacy.USERNAME.value
    ...
    def get_fields(self, safe=False, update_cache_data=False, rest_client=None) -> dict:
        ...
        for user_fields in users_fields:
            if user_fields[UserFieldsNamesLegacy.ID.value] == self.id:
...
def get_user_fields_by_username(rest_client: RestCommands, username, organization_name, safe=False) -> dict:
    ...
    for user_fields in users_fields:
        if user_fields[UserFieldsNamesLegacy.USERNAME.value] == username:
...
def _compare_new_user_data(expected_data: dict, actual_data: dict):
    username = expected_data[UserFieldsNamesLegacy.USERNAME.value]
    logger.info(f"Validate that user {username} created with the correct values as were passed")
    not_relevant_fields = [UserFieldsNamesLegacy.PASSWORD.value, UserFieldsNamesLegacy.ROLES.value]
    ...
    expected_roles = expected_data[UserFieldsNamesLegacy.ROLES.value]
    actual_roles = actual_data[UserFieldsNamesLegacy.ROLES.value]
