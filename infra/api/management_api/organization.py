...
class Organization(BaseApiObj):
    ...
    def get_servers_licences_in_use(self, from_cache=None, update_cache=True):
        field_name = OrgFieldsNames.SERVERS_IN_USE.value
        value = self._get_field(field_name=field_name, from_cache=from_cache, update_cache=update_cache)
        return value
    ...
    @allure.step("Update organization name")
    def update_name(self, new_name: str):
        logger.info(f"Update {self} name to {new_name}")
        new_fields = [
            (OrgFieldsNames.ORG_NAME.value, new_name)
        ]
        self.update_fields(new_fields=new_fields)
