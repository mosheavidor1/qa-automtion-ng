...
class ManagementUiClient:
    ...
    def __init__(self,
        ...
        self.communication_control_app = self.FortiEdrCommControlApp(self)
        self.communication_control_policy = self.CommControlPolicy(self)
        ...
        self.licenses = self.FortiEdrLicenses(self)
    ...
    def start_testim_flow(self,
        ...
        output = self._testim_handler.run_test(test_name=test_name,
                                      ui_ip=management_ui_ip,
                                      data=base_data,
                                      assert_type=assert_type,
                                      test_timeout=test_timeout,
                                      return_cli_output=return_cli_output)
        return output
    ...
    class FortiEdrSecurityEvents:
        ...
        def find_event_and_validate_if_blocked(self, data: dict = None,
                                               assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                               test_timeout: int = 600):
            self.parent.start_testim_flow(test_name="Security event | find and validate event",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)
    ...
    class FortiEdrInventory:
        ...
        def verify_collector_is_disconnected_expired(self,
                                                     data: dict = None,
                                                     assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                                     test_timeout: int = 600):
            self.parent.start_testim_flow(test_name='Inventory | Verify Collector is disconnected expired',
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)
    ...
    class FortiEdrCollectors:
        ...
        def export_collector_logs(self,
                                  data: dict = None,
                                  assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                  test_timeout: int = 600):
            """
            login to management
            go to inventory > collectors
            select a collector and try to export the collector logs
            verify that the generate logs file was success
            verify that the download process was success
            """
            self.parent.start_testim_flow(test_name="Collectors | Get logs",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)
    ...
    class FortiEdrSecurityPolicies:
        ...
        def set_policies_mode(self, data: dict = None, assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                              test_timeout: int = 600):
            """ Set policies (by name) to given mode simulation/prevention and validate mode changed in UI"""
            self.parent.start_testim_flow(test_name="Security policies | Set policies to given mode and validate",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)
    ...
    class FortiEdrCommControlApp:
        def __init__(self, parent):
            self.parent = parent

        def delete_app_from_management(self,
                                       data: dict = None,
                                       assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                       test_timeout: int = 600):
            self.parent.start_testim_flow(test_name="Communication control | delete application",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)

        def deny_app_default_policy(self, data: dict = None, assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                    test_timeout: int = 600):
            """ Deny app's 'Default' communication policy """
            self.parent.start_testim_flow(test_name="Communication control | deny application in default policy",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)

        def validate_app_vulnerability(self, data: dict = None, assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                       test_timeout: int = 600):
            self.parent.start_testim_flow(test_name="Communication control | check vulnerability for application",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)

    class CommControlPolicy:
        def __init__(self, parent):
            self.parent = parent

        def set_default_policy_to_prevention(self, data: dict = None, assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                             test_timeout: int = 600):
            """ Set the 'default' communication control policy to prevention mode """
            self.parent.start_testim_flow(test_name="Communication control | set default policy to prevention",
                                          management_ui_ip=self.parent.management_ui_ip,
                                          data=data,
                                          assert_type=assert_type,
                                          test_timeout=test_timeout)
    ...
    class FortiEdrLicenses:
        def __init__(self, parent):
            self.parent = parent

        def validate_there_are_free_licenses(self,
                                             data: dict = None,
                                             assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                             test_timeout: int = 600):
            """
              perform login
              Verify there is free license
              """
            self.parent.start_testim_flow(
                test_name="Expired Collector | Validate there are free licenses for the new collector",
                management_ui_ip=self.parent.management_ui_ip,
                data=data,
                assert_type=assert_type,
                test_timeout=test_timeout)

        def checking_licenses_status(self,
                                     data: dict = None,
                                     assert_type: AssertTypeEnum = AssertTypeEnum.HARD,
                                     test_timeout: int = 600):
            """
                  ...
                  Verify there is free license
                  """
            output = self.parent.start_testim_flow(
                test_name="checking licenses status",
                management_ui_ip=self.parent.management_ui_ip,
                data=data,
                assert_type=assert_type,
                test_timeout=test_timeout,
                return_cli_output=True)
            return output
