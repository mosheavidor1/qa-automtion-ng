...
class TestImHandler:
    ...
    def run_test(self,
        ...
        passed = StringUtils.get_txt_by_regex(text=output, regex=r'PASSED:\s+(\d+)', group=1)
        failed = StringUtils.get_txt_by_regex(text=output, regex=r'FAILED:\s+(\d+)', group=1)
        evaluating = StringUtils.get_txt_by_regex(text=output, regex=r'EVALUATING:\s+(\d+)', group=1)
        aborted = StringUtils.get_txt_by_regex(text=output, regex=r'ABORTED:\s+(\d+)', group=1)
        skipped = StringUtils.get_txt_by_regex(text=output, regex=r'SKIPPED:\s+(\d+)', group=1)
        duration = StringUtils.get_txt_by_regex(text=output, regex=r'Duration:\s+(\S+)', group=1)
        ...
        if status_code != 0 and passed is not None and passed.isdigit():
            if int(passed) == 0:
                ...
                if assert_type == AssertTypeEnum.SOFT:
                    Assertion.add_message_soft_assert(message=fr"The test step \ test from TestIM: {test_name} has failed")
