...
@allure.epic("Management")
@allure.feature("Inventory - collector logs")
@pytest.mark.e2e_windows_collector_sanity
@pytest.mark.e2e_linux_collector_sanity
@pytest.mark.e2e_windows_collector_full_regression
@pytest.mark.e2e_linux_collector_full_regression
...
def test_generate_collector_logs(management, collector):
    """
    This test checks generate and export collector logs (remote operation) via UI
    Steps: (all in one flow by Testim)
            login to management
            go to inventory > collectors
    ...
    with TEST_STEP(f"STEP - export logs from collector {rest_collector}"):
        management.ui_client.collectors.export_collector_logs(data=test_im_params)
