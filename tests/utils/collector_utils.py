...
class CollectorUtils:
    ...
    def validate_collector_installed_successfully(tenant: Tenant, collector_agent: CollectorAgent, expected_version,
        ...
        if isinstance(collector_agent, LinuxCollector) and expected_package_name is not None:
    ...
    def wait_until_rest_collector_is_off(rest_collector: RestCollector):
        ...
        if Version(collector_version) > Version('6.0.0.0') and "windows" in os.lower():
...
def is_config_file_received_in_collector(collector: CollectorAgent,
    ...
    if desired_config_type == CollectorConfigurationTypes.FULL.value:
        is_found_in_logs = collector.is_string_in_logs(string_to_find=full_string,
                                                       first_log_date_time=first_log_date_time)
        Reporter.report(f'"{full_string}" is found in logs', logger_func=logging.info)
        if is_found_in_logs:
        ...
        return is_found_in_logs

    elif desired_config_type == CollectorConfigurationTypes.PARTIAL.value:
        is_found_in_logs = collector.is_string_in_logs(string_to_find=partial_string,
                                                       first_log_date_time=first_log_date_time)
        Reporter.report(f'"{partial_string}" is found in logs', logging.info)
        if is_found_in_logs:
            Reporter.report(f'"{partial_string}" is found in logs')
...
def get_previous_build_version_number(collector: CollectorAgent):
    ...
    base_version = StringUtils.get_txt_by_regex(text=current_version, regex=r'(\d+.\d+.\d+).\d+', group=1)
    ...
    elif 'centos' in collector.os_station.distro_data.version_name.lower():
        if 'centos6' in collector.os_station.distro_data.version_name.lower():
            builds_list = builds_dict.get('centos_6_collector')
        elif 'centos7' in collector.os_station.distro_data.version_name.lower():
            builds_list = builds_dict.get('centos_7_collector')
        elif 'centos8' in collector.os_station.distro_data.version_name.lower():
...
@contextmanager
def wait_till_configuration_drill_down_to_collector(collector_agent: CollectorAgent):
    if collector_agent is not None:
        config_files_details_before_config_operation = collector_agent.get_configuration_updated_files_details()
        yield
        collector_agent.wait_for_new_config_file(config_files_details_before_action=config_files_details_before_config_operation)
    else:
        Reporter.report(
            "Skipping validation of configuration update in collector since CollectorAgent object is None - "
            "No need to worry, maybe the test don't need to check it")
