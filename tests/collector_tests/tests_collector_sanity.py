...
@pytest.mark.basic_functionality_windows_collector
@pytest.mark.basic_functionality_linux_collector
...
def test_uninstall_windows_collector_via_remote_ops(management, aggregator, collector):
    ...
    """
    is_EN_80924_resolved = False
    if not is_EN_80924_resolved and Version(collector.get_version()) > Version('6.0.0.0'):
        assert False, "http://jira/browse/EN-80924 is not resolved yet, failing this test"
...
def test_collector_with_new_org_registration_password(management, aggregator, collector):
    ...
    with new_tenant_context(management=management,
                            ...
                            move_collector_to_new_org=True) as tenant_with_collector:
